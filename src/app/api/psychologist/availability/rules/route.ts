...
export async function POST(req: NextRequest) {
  try {
    ...
    const rules = await AvailabilityService.setAvailabilityRules(profile.id, body);
    return NextResponse.json({ success: true, message: "Availability rules updated", data: rules });
  } catch (error) {
